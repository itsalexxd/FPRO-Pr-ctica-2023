...
public class practicaGabriel {
    ...
    public static void main(String[] args)throws FileNotFoundException{ 
        // Definimos el  nombre del archivo y su ruta (Solo escribimos la ruta si está en un directorio diferente al del programa);
        ...
        if(textoTableros.exists() == true){
            ...
            do{
                ...
                while(tableroCompletado == false){
                    ...
                    else if(casillaBloqueada(tableroInicial, columna, fila) == true){ // Si la casilla pertenece al tablero inicial
                        imprimirTablero(tablero); 
                        System.out.print("               ");
                        System.out.println("La casilla esta bloqueada ya que pertenece al tablero inicial. Vuelva a introducir la jugada");
                    }
            ...
            if(numeroFichasEsIgual(tablero) == true && filasIguales(tablero) == false && columnasIguales(tablero) == false && comprobar3(tablero) == true){
            ...
            if(otra.equals("SI") == true){  
                ...
                System.out.println("Has ganado "+tablerosGanados+" tablero/s lo que te da un porcertaje de victorias del "+winrate+" %.");
            }    
    ...
    public static int contarLineasTexto(Scanner scNumLineasTXT){  // Contar el número de líneas que tiene el archivo .txt
        int contador = 0;
        while(scNumLineasTXT.hasNextLine() == true){  // Verifica que haya otra línea, si la hay entonces...

            contador++; // ... suma una línea más al contador (Más tarde podemos guardar las líneas en un array y usar el contador para definir el tamaño del array )
            scNumLineasTXT.nextLine();
        }
        return contador;
    }
    public static String[] codigosTableroTexto(int numLineasTXT, Scanner scLeerTXT){  // Guarda las lineas del archivo de texto en un array
    ...
    public static int[][] definirTableroGenerado(String tableroGenerado){  // Define el tablero para los valores del string inicial (Tamaño fijado del tablero 6x6)
    ...
    public static int leerFila(int[][]tablero, String movimiento){ // Devuelve el valor de la fila si el string es correcto.
        int fila = -1; 
    ...
    public static int leerColumna(int[][]tablero, String movimiento){ // Devuelve el valor de la columna si el string es correcto.
        int columna = -1;
        for( int i = 0; i < movimiento.length();i++ ){
    ...
    public static void imprimirTablero(int[][]tablero){  // Imprime el tablero INICIAL
        int filasTotal = tablero.length; int columnasTotal = tablero.length;
    ...
    public static boolean movimientoSinFallos(String movimiento){ // Comprueba que el jugador ha escrito bien el movimiento 
        boolean sinError = true; boolean esNumero = false; boolean esLetra = false;
        if(movimiento.length() != 2){
            ...
            for(int i = 0; i < movimiento.length();i++){
                ...
                else if( ( (movimiento.charAt(i) >= 'a' && movimiento.charAt(i) <= 'f') || (movimiento.charAt(i) >= 'A' && movimiento.charAt(i) <= 'F') ) && esLetra == false){
                    esLetra = true;
                }
                else{
                    return sinError = false;
    ...
    public static boolean casillaBloqueada(int[][]tableroInicial, int leerColumna, int leerFila){ // Bloquea las casillas del tablero inicial
    ...
    public static int[][] jugadaAnterior(int[][][]tablerosAnteriores, int[][]tablero, int contadorMovimientos){ // Devuelve el tablero con la "jugada" anterior del usuario.
        for(int fila = 0; fila  < tablero.length ;fila++){
            for(int columna = 0; columna < tablero.length; columna++){
    ...
    public static boolean numeroFichasEsIgual(int[][]tablero){ // Comprueba que haya el mismo número de cada tipo de fichas en todas las  filas y columnas.
        boolean tableroCorrecto = true;
        for(int i = 0; i < tablero.length && tableroCorrecto == true;i++){ // Comprobamos las filas y las columnas
            int aspasFila = 0; int circulosFila = 0; int aspasColumna = 0; int circulosColumna = 0;
            for(int j = 0; j < tablero.length; j++){
    ...
    public static boolean comprobar3 (int[][] tablero){ // No puede haber más de dos valores iguales contiguos, en horizontal ni en vertical.
